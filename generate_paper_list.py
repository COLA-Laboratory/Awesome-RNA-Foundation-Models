...
# (name, paper_title, paper_url, year_month, github_url, hf_url, category, abstract)
papers = [
    # === ncRNA FMs ===
...
for name, title, url, ym, gh, hf, cat, abstract in papers:
    if cat not in categories:
        categories[cat] = []
    categories[cat].append((name, title, url, ym, gh, hf, abstract))
...
# Find and replace existing Paper List section
paper_list_start = readme.find("\n## Paper List")
contributing_start = readme.find("\n## Contributing")

if paper_list_start != -1 and contributing_start != -1:
    # Replace existing Paper List
    new_readme = readme[:paper_list_start] + output + "\n" + readme[contributing_start:]
elif contributing_start != -1:
    # Insert before Contributing
    new_readme = readme[:contributing_start] + output + "\n" + readme[contributing_start:]
else:
