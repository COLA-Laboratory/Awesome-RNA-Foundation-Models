...
# newly pre-trained RNA sequence FM, a narrower RNA-specific FM, or a
# derivative/adaptation that still yields a reusable RNA sequence model.
scope_labels = {
    ...
    "rna_inclusive_broad": "Broad RNA-Inclusive Foundation Models",
}
...
scope_descriptions = {
    ...
    "rna_inclusive_broad": "The model is not RNA-only, but its pre-training corpus explicitly includes RNA / transcript sequences and the checkpoint supports reusable sequence representations, fine-tuning, or generation relevant to RNA tasks.",
}
def scope_key(paper):
    name = paper[0]
...
rna_type_labels = {
    ...
    "DNA+RNA FM": "Broad RNA-Inclusive Nucleotide / Multi-modal Models",
}
...
model_table_descriptions = {
    ...
    "DNA+RNA FM": "Broad sequence foundation models whose pre-training explicitly includes RNA or transcript sequences alongside DNA, protein, or other biological sequence modalities.",
}
...
def render_model_timeline():
    ...
    rows.append("")
    rows.append(f"Timeline of {len(papers)} confirmed RNA sequence foundation model entries, generated from `data/papers.yaml` whenever confirmed metadata is refreshed.")
    rows.append("")
...
lines.append("")
lines.append("A survey list of RNA sequence foundation models and RNA-inclusive sequence foundation models included in this survey. Each entry shows the model/resource name separately from the official paper title. Four classification views are provided below — click to expand/collapse each view.")
lines.append("")
...
lines.append("- **Broad RNA-Inclusive Foundation Models**: broader nucleotide, biological-sequence, central-dogma, or multi-modal foundation models whose pre-training explicitly includes RNA / transcript sequences and whose checkpoints support reusable sequence representations, fine-tuning, or generation for RNA-relevant tasks.")
...
lines.append('<summary><b>RNA Sequence Foundation Models</b></summary>')
lines.append("")
...
if generated_start != -1 and next_section != -1:
    ...
    header = f"""# Awesome RNA Foundation Models
...
A curated, continuously updated catalog of **RNA sequence foundation models**, covering reusable RNA/mRNA/CDS/UTR sequence backbones and checkpoints, specialized RNA-family or structure-aware sequence models, adapted / derived RNA language models, generative RNA sequence models, and broader sequence foundation models with explicit RNA/transcript sequence pre-training signal.
...
new_readme = new_readme.replace(
    "A curated, continuously updated catalog of **RNA and RNA-relevant foundation models**, covering reusable RNA/mRNA/CDS/UTR sequence backbones, task-oriented RNA design and prediction models, structure-aware RNA models, RNA-related nucleotide or multi-omics models, and RNA-seq / transcriptomic models.",
    "A curated, continuously updated catalog of **RNA sequence foundation models**, covering reusable RNA/mRNA/CDS/UTR sequence backbones and checkpoints, specialized RNA-family or structure-aware sequence models, adapted / derived RNA language models, and generative RNA sequence models.",
)
new_readme = new_readme.replace(
...
new_readme = new_readme.replace(
    "A curated, continuously updated catalog of **RNA foundation models**, covering reusable RNA/mRNA/CDS/UTR sequence backbones and checkpoints, RNA-trained design and prediction models, structure-aware RNA models, RNA-related nucleotide or multi-omics foundation models with explicit RNA/transcriptomic training signal, and RNA-seq / transcriptomic foundation models.",
    "A curated, continuously updated catalog of **RNA sequence foundation models**, covering reusable RNA/mRNA/CDS/UTR sequence backbones and checkpoints, specialized RNA-family or structure-aware sequence models, adapted / derived RNA language models, and generative RNA sequence models.",
)
new_readme = re.sub(
    r"^# .*Awesome RNA Foundation Models.*$",
    "# Awesome RNA Foundation Models",
    new_readme,
    count=1,
    flags=re.M,
)
if "[![Awesome](https://awesome.re/badge.svg)](https://awesome.re)" not in new_readme[:500]:
    new_readme = new_readme.replace(
        "# Awesome RNA Foundation Models\n\n",
...
new_readme = re.sub(
    r"\[!\[Awesome\]\(https://awesome\.re/badge\.svg\)\]\(https://awesome\.re\)\n\n"
    r"\[!\[MIT License\]\(https://img\.shields\.io/badge/license-MIT-green\.svg\)\]\(https://opensource\.org/licenses/MIT\)\n"
    r"\[!\[Maintenance\]\(https://img\.shields\.io/badge/Maintained%3F-yes-green\.svg\)\]\(https://github\.com/\)\n"
    r"\[!\[PR's Welcome\]\(https://img\.shields\.io/badge/PRs-welcome-brightgreen\.svg\?style=flat\)\]\(http://makeapullrequest\.com\)\n"
    r"\[!\[Last Update\]\(https://img\.shields\.io/badge/Last_Update-2026\.05-blue\.svg\)\]\(\)",
    "[![Awesome](https://awesome.re/badge.svg)](https://awesome.re) [![MIT License](https://img.shields.io/badge/license-MIT-green.svg)](https://opensource.org/licenses/MIT) [![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-green.svg)](https://github.com/) [![PR's Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg?style=flat)](http://makeapullrequest.com) [![Last Update](https://img.shields.io/badge/Last_Update-2026.05-blue.svg)]()",
    new_readme,
)
new_readme = re.sub(
...
new_readme = new_readme.replace(
    "- [Model Timeline](#model-timeline) — Auto-generated timeline for confirmed RNA foundation model entries",
    "- [Model Timeline](#model-timeline) — Auto-generated timeline for confirmed RNA sequence foundation model entries",
)
new_readme = new_readme.replace(
...
new_readme = new_readme.replace(
    "- [Paper List](#paper-list) — RNA foundation models (4 views), Benchmarks, Surveys",
    "- [Paper List](#paper-list) — RNA sequence foundation models (4 views), Benchmarks, Surveys",
)
new_readme = re.sub(
